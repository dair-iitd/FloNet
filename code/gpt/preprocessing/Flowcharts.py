...
class Flowcharts(object):
    ...
    def __init__(self, flowchartJsons):
        self._flowchartJsons = flowchartJsons
        self._populate_node_properties()

    def get_node_utterance(self, chart, node):
        return chart['nodes'][node]['utterance']
    def get_all_tuples(self, name):
        tuples = []
        chart=self._flowchartJsons(name)
        for node, edges in chart['edges'].items():
            for edge, child in edges.items():
                tuples.append((self.get_node_utterance(chart,node),self.get_node_utterance(chart,child),edge))
    ...
    def get_terminals(self, flowchart):
        flowchart = self._flowchartJsons[flowchart]
